...
while [[ $# -gt 0 ]]; do
	...
	-t | --type)
		...
		echo "Using $DOCKER_TYPE build type"
		;;
	-r | --repo)
		# Check if has argument value
		if [[ -z "$2" ]] || [[ "$2" =~ ^-+ ]]; then
			echo ""${DOCKER_IMAGE_NAME}": '$1' missing repository url argument" >&2
...
if [[ $HELP == true ]]; then
	...
	echo "docker run -v  "$(pwd)/test":/app    "${DOCKER_IMAGE_NAME}":latest  -r https://github.com/jasonyang-ee/STM32-CMAKE-TEMPLATE.git -t Release"
        echo "docker run -v  "$(pwd)/test2":/app    "${DOCKER_IMAGE_NAME}":latest  -r https://github.com/Teivaz/cmake-stm32.git  -t Release "
	echo ""
...
if [[ $GITHUB_ACTIONS == true ]]; then
	DOCKER_INPUT_PATH=$GITHUB_WORKSPACE
else
	mkdir -p $DOCKER_INPUT_PATH
fi
...
if [[ ! -z $REPO ]]; then
  ...
  #git clone $REPO $DOCKER_INPUT_PATH  –build-arg SSH_PRIVATE_KEY=”$(cat ~/.ssh/id_dsa)”  
echo "ssh-agent"
ssh-agent $(ssh-add  ~/.ssh/id_dsa; git clone $REPO $DOCKER_INPUT_PATH)

  
fi

# Check if project exists in volume
if [[ -z "$(ls -A $DOCKER_INPUT_PATH)" ]]; then
	echo "Volume $DOCKER_INPUT_PATH is empty"
...
CORES=$(nproc)
cmake -DCMAKE_BUILD_TYPE=$DOCKER_TYPE -S $DOCKER_INPUT_PATH -B $DOCKER_INPUT_PATH/build/ -G Ninja
cmake --build $DOCKER_INPUT_PATH/build/ -j$CORES
if [[ $? -eq 0 ]]; then
	echo '                                            ^'
